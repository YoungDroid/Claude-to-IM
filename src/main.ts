...
import { initBridgeContext } from './lib/vendor/bridge/context.js';
import * as bridgeManager from './lib/vendor/bridge/bridge-manager.js';
// Side-effect import to trigger adapter self-registration
import './lib/vendor/bridge/adapters/index.js';

import type { LLMProvider } from './lib/vendor/bridge/host.js';
  ...
  if (runtime === 'auto') {
    ...
    if (cliPath) {
      ...
      if (check.ok) {
        ...
        return new SDKLLMProvider(pendingPerms, cliPath, config.autoApprove, config.dangerouslySkipPermissions);
      }
