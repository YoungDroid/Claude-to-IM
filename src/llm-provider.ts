...
import type { LLMProvider, StreamChatParams, FileAttachment } from './lib/vendor/bridge/host.js';
 ...
 * Detects PDFs by magic bytes even when reported as application/octet-stream.
 * Returns a string like "\n\n[PDF: filename]\n<extracted text>"
 */
async function extractPdfText(files?: FileAttachment[]): Promise<string> {
  const pdfFiles = files?.filter(f => f.type === 'application/pdf' || f.type === 'application/octet-stream');
  ...
  for (const pdf of pdfFiles) {
    try {
      ...
      if (!isPdf(buffer)) continue; // not a PDF, skip
      const parsed = await pdfParse(buffer);
      const trimmed = parsed.text.trim();
      if (trimmed) {
        parts.push(`\n\n[PDF: ${pdf.name}]\n${trimmed}`);
      } else {
        parts.push(`\n\n[PDF: ${pdf.name}] (no extractable text)`);
      }
    } catch (err) {
      console.warn('[llm-provider] Failed to parse PDF:', err instanceof Error ? err.message : String(err));
      parts.push(`\n\n[PDF: ${pdf.name}] (failed to read)`);
    }
  }
  return parts.join('');
}
...
function buildPrompt(
  ...
  const hasPdf = files?.some(f => f.type === 'application/pdf' || f.type === 'application/octet-stream');
  if ((!imageFiles || imageFiles.length === 0) && !hasPdf) {
    return text;
  }

  // Always use async path when PDFs are involved so we can extract text
  return (async function* () {
    const pdfText = await extractPdfText(files);
    const combinedText = pdfText ? `${text}\n${pdfText}` : text;
...
export class SDKLLMProvider implements LLMProvider {
  ...
  constructor(private pendingPerms: PendingPermissions, cliPath?: string, autoApprove = false, dangerouslySkipPermissions = false) {
        ...
        (async () => {
          ...
          try {
            ...
            const queryOptions: Record<string, unknown> = {
              ...
              allowDangerouslySkipPermissions: dangerouslySkipPermissions || undefined,
              includePartialMessages: true,
              env: cleanEnv,
              // Pass bypass permissions config to CLI subprocess settings.
              // This is passed to the CLI as a settings object, not as CLI args.
              ...
              canUseTool: async (
                  ...
                  if (autoApprove || dangerouslySkipPermissions) {
