...
async function handleMessage(
  ...
  // Handle callback queries (permission buttons, interrupt button)
  if (msg.callbackData) {
    // Handle streaming card interrupt button (stop button)
    if (msg.callbackData.startsWith('interrupt:')) {
      const binding = router.resolve(msg.address);
      const st = getState();
      ...
      // Finish the streaming card with 'interrupted' status if adapter supports it
      if (adapter.onStreamEnd) {
        try {
  ...
  // /compact is passed through to the CLI to handle compaction
  if (rawText.startsWith('/') && !rawText.startsWith('/compact')) {
  ...
  switch (command) {
    case '/start':
      response = [
        ...
        '/compact - Compact conversation context (via CLI)',
    ...
    case '/clear': {
      ...
      const oldTask = st.activeTasks.get(binding.codepilotSessionId);
      if (oldTask) {
        oldTask.abort();
        st.activeTasks.delete(binding.codepilotSessionId);
      }
      // 2. Clear SDK session ID to prevent resume
      if (binding.id) {
        router.updateBinding(binding.id, { sdkSessionId: '' });
      }
      // 3. Clear stored messages if store supports it
      try {
        const { store } = getBridgeContext();
        if (store.clearMessages) {
          store.clearMessages(binding.codepilotSessionId);
        }
      } catch { /* store may not support this */ }
      response = 'Context cleared. Ready for a fresh conversation.';
      break;
    }
