...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  async start(): Promise<void> {
    ...
    // Monkey-patch: add cardkit.v2 shim using direct API calls (SDK only exposes v1)
    let _tenantAccessToken = '';
    const getToken = async () => {
      if (_tenantAccessToken) return _tenantAccessToken;
      try {
        const baseUrl = (this.restClient as any).domain || 'https://open.feishu.cn';
    ...
    (this.restClient as any).cardkit.v2 = {
      card: {
        // CardKit v1 create — POST /open-apis/cardkit/v1/cards
        ...
        // CardKit v1 streamContent — PUT /open-apis/cardkit/v1/cards/{id}/elements/{element_id}/content
        ...
        settings: {
          streamingMode: {
            // CardKit v1 settings — PATCH /open-apis/cardkit/v1/cards/{card_id}/settings
        ...
        // CardKit v1 update — PUT /open-apis/cardkit/v1/cards/{card_id}
  ...
  private async _doCreateStreamingCard(chatId: string, replyToMessageId?: string): Promise<boolean> {
    ...
    try {
                  ...
                  elements: [
                    {
                      ...
                      type: 'danger',
                      size: 'small',
                      value: { callback_data: `interrupt:${chatId}` },
                    },
                  ],
                },
              ],
      ...
      if (!cardId) {
        console.warn('[feishu-adapter] Card create failed:', JSON.stringify(createResp).slice(0, 300));
        return false;
      }
