...
case "${1:-help}" in
  start)
    ...
    if [ "$STARTED" = "true" ]; then
      ...
      if [ "${CTI_WATCHDOG_ENABLED:-true}" = "true" ]; then
        echo "Starting watchdog..."
        nohup bash "$SKILL_DIR/scripts/watchdog.sh" > "$CTI_HOME/logs/watchdog.log" 2>&1 &
        sleep 2
        if [ -f "$CTI_HOME/runtime/watchdog.pid" ]; then
          echo "Watchdog started (PID: $(cat "$CTI_HOME/runtime/watchdog.pid"))"
        fi
      fi
    else
  ...
  stop)
    # Stop watchdog first
    WATCHDOG_PID=$(cat "$CTI_HOME/runtime/watchdog.pid" 2>/dev/null || true)
    if [ -n "$WATCHDOG_PID" ] && kill -0 "$WATCHDOG_PID" 2>/dev/null; then
      echo "Stopping watchdog..."
      kill "$WATCHDOG_PID" 2>/dev/null || true
      sleep 2
      kill -0 "$WATCHDOG_PID" 2>/dev/null && kill -9 "$WATCHDOG_PID" 2>/dev/null || true
      rm -f "$CTI_HOME/runtime/watchdog.pid"
    fi
  ...
  status)
    ...
    # Watchdog status
    if [ -f "$CTI_HOME/runtime/watchdog.pid" ]; then
      WPID=$(cat "$CTI_HOME/runtime/watchdog.pid")
    ...
    if [ -f "$CTI_HOME/runtime/watchdog-stats.json" ]; then
      echo "Watchdog stats:"
      cat "$CTI_HOME/runtime/watchdog-stats.json" 2>/dev/null
    fi
    ;;
  ...
  watchdog)
    if [ ! -f "$CTI_HOME/config.env" ]; then
      echo "No config.env found at $CTI_HOME/config.env"
      exit 1
    fi
    set -a && source "$CTI_HOME/config.env" && set +a
    if [ "${CTI_WATCHDOG_ENABLED:-true}" != "true" ]; then
      echo "Watchdog is disabled (CTI_WATCHDOG_ENABLED != true)"
      exit 1
    fi
    exec bash "$SKILL_DIR/scripts/watchdog.sh"
    ;;

  *)
    echo "Usage: daemon.sh {start|stop|status|logs [N]|watchdog}"
    ;;
esac
